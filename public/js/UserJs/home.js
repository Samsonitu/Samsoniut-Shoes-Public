// Begin scroll header
    ...
    ? 'background-color: #03177e; position: fixed;' 
    : 'background-color: transparent; position: fixed; ';
    header.style = lastScrollTop;

    window.addEventListener('scroll', () => {
        const currentColor = window.pageYOffset > 0 
        ? 'background-color:#03177e; position: fixed;' 
        : 'background-color: transparent; position: fixed;'
        if (currentColor !== lastScrollTop) {
            header.style = currentColor;
            lastScrollTop = currentColor;
        }
    });
// End scroll header
