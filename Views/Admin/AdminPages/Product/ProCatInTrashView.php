<?php 
    $title = "Thùng Rác Danh Mục Sản Phẩm | SSNT SHOES";
    $activePage = [
    ...
    $extraJS = [
        ...
        public_dir('js/AdminJs/Product/proCatInTrash.js'),
    ];
...
<div class="content">
    <div class="bread-crump">
        <p>
            ...
            <a href="<?= route('AdminProCatInTrashRoute'); ?>">Thùng Rác Danh Mục Sản Phẩm</a>
        </p>
    </div>
    ...
    <table>
        ...
        <tbody>
            <?php if (!empty($proCatInTrashList)) : ?>
                <?php foreach ($proCatInTrashList as $index => $category) : ?>
                    <tr data-pro-cat-id="<?= $category['catId'] ?>" data-pro-cat-name= "<?= $category['catName'] ?>">
                        <td><?= $index + 1; ?></td>
    ...
    <div class="modal fade" id="modalDelProCatPermanently" 
        tabindex="-1" aria-labelledby="modalDelProCatPermanentlyLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    <form action="<?= route('AdminRemoveProductCategoryRoute'); ?>" method="post">
                        <input type="hidden" name="proCatIdDelete">
                        <button type="submit" name="SubmitDeleteProductCategory" class="btn border-3 border-danger" value="SubmitDelete"><b>Xóa vĩnh viễn</b> danh mục 
                            <b id="proCatNameDelete"></b> 
                        </button>
                    </form>
                    </div>
                </div>
    ...
    <div class="modal fade" id="modalRestoreProCatInTrash" 
        tabindex="-1" aria-labelledby="modalRestoreProCatInTrashLabel" aria-hidden="true">
        <div class="modal-dialog modal-sm">
            <div class="modal-content">
                <div class="text-end p-2"><button type="button" class="btn btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body text-center">
                    <form action="<?= route('AdminRestoreProCatInTrashRoute'); ?>" method="post">
                        <input type="hidden" name="proCatIdRestore">
                        <button type="submit" name="SubmitRestoreProCatInTrash" class="btn border-3 border-success" value="SubmitChange"><b>Khôi phục </b> danh mục 
                            <b id="proCatNameRestore"></b> 
                        </button>
                    </form>
                    </div>
                </div>
